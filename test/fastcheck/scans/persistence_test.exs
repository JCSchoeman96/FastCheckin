defmodule FastCheck.Scans.PersistenceTest do
  ...
  test "persist_batch does not project legacy check-in state for successful out results" do
    event = create_event()

    attendee =
      create_attendee(event, %{
        ticket_code: "PERSISTOUT001",
        allowed_checkins: 1,
        checkins_remaining: 1,
        payment_status: "completed"
      })
    ...
    now = DateTime.utc_now() |> DateTime.truncate(:second)
    result = out_success_result(event.id, attendee.id, attendee.ticket_code, "persist-out-1", now)
    ...
    assert Repo.aggregate(
             from(check_in in CheckIn,
               where:
                 check_in.event_id == ^event.id and
                   check_in.ticket_code == ^attendee.ticket_code
             ),
             :count,
             :id
           ) == 0
    ...
    persisted_attendee = Repo.get!(Attendee, attendee.id)
    assert persisted_attendee.checkins_remaining == 1
    assert persisted_attendee.checked_in_at == nil
  end
  ...
  defp out_success_result(event_id, attendee_id, ticket_code, idempotency_key, now) do
    %{
      event_id: event_id,
      attendee_id: attendee_id,
      idempotency_key: idempotency_key,
