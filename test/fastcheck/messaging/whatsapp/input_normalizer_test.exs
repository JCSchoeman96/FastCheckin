defmodule FastCheck.Messaging.WhatsApp.InputNormalizerTest do
  ...
  describe "normalize/1" do
    test "normalizes canonical positive integer commands" do
      ...
      assert {:ok, {:number, 99}} = InputNormalizer.normalize("99")
      assert {:ok, {:number, 100}} = InputNormalizer.normalize("100")
      assert {:ok, {:number, 12}} = InputNormalizer.normalize(" 12 ")
    ...
    test "rejects blank, non-text, noncanonical numeric, and oversized input" do
      assert {:error, :blank} = InputNormalizer.normalize("  ")
      assert {:error, :invalid} = InputNormalizer.normalize(nil)
      assert {:error, :invalid} = InputNormalizer.normalize("00")
      assert {:error, :invalid} = InputNormalizer.normalize("01")
      assert {:error, :invalid} = InputNormalizer.normalize("0012")
