defmodule FastCheck.Messaging.WhatsApp.ProviderStatus do
  ...
  @max_raw_payload_hash_length 128
  ...
  def normalize(payload, opts) when is_map(payload) do
    ...
    raw_payload_hash = safe_raw_payload_hash(Keyword.get(opts, :raw_payload_hash, ""))
  ...
  defp parse_timestamp(timestamp) when is_integer(timestamp), do: from_unix_timestamp(timestamp)
  ...
  defp safe_error_code(code) when is_binary(code) do
    ...
    if byte_size(code) <= @max_error_code_length and
  ...
  defp safe_raw_payload_hash(value) when is_binary(value) do
    value = String.trim(value)

    if byte_size(value) <= @max_raw_payload_hash_length, do: value, else: ""
  end

  defp safe_raw_payload_hash(_value), do: ""
